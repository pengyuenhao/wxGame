...
import { WebSocketOption } from "../config";
import * as http from "http";
import * as https from "https";
import * as websocket from "ws";
import { IConverter, GetType, GetConverter, GetCtorByValue, GetTypeByValue, Writer, GetSerial, Reader, GetCtor, GetSerialByValue } from "peng-field";
...
export class WebSocketUtil extends Singleton {
    ...
    public static Credentials = {
        key: readFileSync('../private/ssl/215021224260713.key'),
        cert: readFileSync('../private/ssl/215021224260713.pem')
    }
    ...
    /**初始化 */
    public async init(option: WebSocketOption) {
        let server;
        if (option.port === 443) {
        ...
        return new Promise<Boolean>((resolve) => {
            this.server.on("connection", (socket: WebSocket, request: http.IncomingMessage) => {
                console.log("[Socket]" + "<链接成功>" + request.headers.origin + request.headers.host + "[端口]" + (<any>socket)._socket.localPort + "/" + (<any>socket)._socket.remotePort);
    ...
    private CallbackClose(socket: WebSocket, ev: CloseEvent) {
        console.log("[Socket]" + "<链接断开>" + ev.code + "[端口]" + (<any>socket)._socket.remotePort);
    }
    private CallbackMessage(event: MessageEvent) {
        ...
        let converter = GetConverter(info.serial);
    ...
    */
    public request<T>(cls: new (...any: any[]) => T, token: any, isOnce: boolean = true): Promise<T> {
        ...
        return new Promise<T>((resolve) => {
            ...
            let info: PackInfo;
            let type = GetSerial(cls);
            let serial = GetSerialByValue(token);
            //这里使用指定的序列号和时间来发送
    ...
    public response<T>(cls: new (...any: any[]) => T, callback: (token: any, reply: (result: T) => void, ws?: WebSocket) => void) {
        ...
        this.listen(cls, (data, ws, index) => {
            callback(data, (result) => {
                //根据回复的数据类型指定序列值
                let type = GetSerial(cls);
                let serial = GetSerialByValue(result);
                //这里使用指定的序列号和时间来发送
                let response = <ResponseBinding>{
                    index: index,
                    type: type,
    ...
    public send(data: any, response?: ResponseBinding, ws?: WebSocket) {
        ...
        let converter = GetConverter(serial);
...
class ClassBinding {
    ...
    public converter: IConverter<any>;
}
/**
 * 封包信息
 */
class PackInfo {
    ...
    public serial: number;
    public static pack(info: PackInfo): Writer {
        let writer: Writer = Writer.create();
    ...
    public static unpack(data: Uint8Array, info: PackInfo): Reader {
