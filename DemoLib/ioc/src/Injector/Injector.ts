...
import { DecoratorClassBinding } from "..";

export class Injector {
    public factory: InjectFactory = null;
    public binder: InjectBinder<InjectBinding> = null;
    public injectClassBinder: DecoratorClassBinder<DecoratorClassBinding> = null;

    constructor() {
        this.factory = new InjectFactory();
    }
    public uninject(target,userClass? : new()=>any) {
        ...
        //获取注入类
        let injectClass: DecoratorClass = this.injectClassBinder.get(userClass);
    ...
    private tryInject(binding: InjectBinding, target: any) {
        //如果工厂不能创建实例则这里直接返回
        if (target != null) {
            if (binding.isInject()) {
    ...
    public inject(target: any, userClass ? : new()=>any): any {
        if (!this.binder || !target) throw new Error("Attempt to inject into Injector without a Binder or null instance");
        //排除一些不能被注入的类型
        let type: any = typeof target;
        if (type === "string" || type === "boolean" || type === "number" || type === "symbol" || type === "undefined" || type === "function") {
    ...
    private decoratorInject(target: any, injectClass: DecoratorClass) {
        ...
        injectClass.list.forEach((binding) => {
            ...
            if (injectBinding) {
                //检查是否有循环依赖
/*                 let loop = injectClass.loop.get(injectBinding.key);
                if(loop){
                    let value = loop.get(injectBinding.name);
                    if(value != null){
