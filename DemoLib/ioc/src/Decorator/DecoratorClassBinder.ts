...
export class DecoratorClassBinder<T> extends Binder<T> {
    ...
    //获取需要注入的类型数据
    public get(target: new()=>any): DecoratorClass {
        //检查是否为实例
        ...
        //从缓存映射中获取对应的列表
        let injectClass : DecoratorClass = this._decoratorClassBufferMap.get(ctor)
        //检查是否存在缓存
        if (!injectClass) {
            //获取继承列表
